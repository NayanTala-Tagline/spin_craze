...
import 'package:spin_craze/features/auth_module/provider/sc_auth_provider.dart';
...
class ScLoginScreen extends StatelessWidget {
  const ScLoginScreen({super.key});

  @override
  Widget build(BuildContext context) {
    AnalyticsManager.instance.logScreenView(
      screenName: 'login',
      screenClass: 'ScLoginScreen',
    );
    return AnnotatedRegion<SystemUiOverlayStyle>(
      value: _statusBarStyle,
      child: ChangeNotifierProvider(
        create: (_) => ScAuthProvider(),
        child: const _ScLoginBody(),
      ),
    );
  }
}

class _ScLoginBody extends StatefulWidget {
  const _ScLoginBody();

  @override
  State<_ScLoginBody> createState() => _ScLoginBodyState();
}

class _ScLoginBodyState extends State<_ScLoginBody>
  ...
  Widget build(BuildContext context) {
    return DecoratedBox(
      ...
      child: Stack(
        children: [
          const Positioned.fill(child: _ScFloatingParticles()),
          Scaffold(
...
class _ScAnimatedLogo extends StatefulWidget {
  const _ScAnimatedLogo();

  @override
  State<_ScAnimatedLogo> createState() => _ScAnimatedLogoState();
}

class _ScAnimatedLogoState extends State<_ScAnimatedLogo>
    with TickerProviderStateMixin {
          ...
          AnimatedBuilder(
            ...
            builder: (context, _) {
              return CustomPaint(
                ...
                painter: _ScHaloPainter(phase: _haloCtrl.value),
              );
            },
          ),
          AnimatedBuilder(
...
class _ScHaloPainter extends CustomPainter {
  _ScHaloPainter({required this.phase});
  ...
  @override
  bool shouldRepaint(covariant _ScHaloPainter oldDelegate) =>
      oldDelegate.phase != phase;
}

// ── Tagline (WATCH ◇ EARN ◇ PLAY) ───────────────────────────────────────────

class _ScTagline extends StatelessWidget {
  const _ScTagline({
    required this.watch,
    required this.earn,
    required this.play,
...
class _ScDiamond extends StatelessWidget {
  const _ScDiamond();
...
class _ScWelcomeCard extends StatelessWidget {
  const _ScWelcomeCard({
    required this.title,
        ...
        children: [
          ...
          FadeTransition(
            ...
            child: SlideTransition(
              ...
              child: Consumer<ScAuthProvider>(
                builder: (context, auth, _) => _ScPrimaryButton(
          ...
          FadeTransition(
            ...
            child: SlideTransition(
              position: guestSlide,
              child: Consumer<ScAuthProvider>(
                builder: (context, auth, _) => _ScSecondaryButton(
  ...
  Future<void> _handleGoogle(BuildContext context, ScAuthProvider auth) async {
  ...
  Future<void> _handleGuest(BuildContext context, ScAuthProvider auth) async {
    AnalyticsManager.instance.logEvent(
      name: 'login_attempt',
...
class _ScPrimaryButton extends StatefulWidget {
  const _ScPrimaryButton({
    required this.label,
  ...
  @override
  State<_ScPrimaryButton> createState() => _ScPrimaryButtonState();
}

class _ScPrimaryButtonState extends State<_ScPrimaryButton>
...
class _ScSecondaryButton extends StatefulWidget {
  const _ScSecondaryButton({
    required this.label,
    required this.leading,
    required this.onPressed,
    this.isLoading = false,
  ...
  @override
  State<_ScSecondaryButton> createState() => _ScSecondaryButtonState();
}

class _ScSecondaryButtonState extends State<_ScSecondaryButton> {
  bool _pressed = false;
...
class _ScFooter extends StatelessWidget {
  const _ScFooter({
...
class _ScFloatingParticles extends StatefulWidget {
  const _ScFloatingParticles();

  @override
  State<_ScFloatingParticles> createState() => _ScFloatingParticlesState();
}

class _ScFloatingParticlesState extends State<_ScFloatingParticles>
    with SingleTickerProviderStateMixin {
  late final AnimationController _ctrl;
  late final List<_ScParticle> _particles;
  ...
  void initState() {
    ...
    _particles = List.generate(16, (_) => _ScParticle.random(rng));
  }
...
class _ScParticle {
  ...
  factory _ScParticle.random(math.Random rng) {
    return _ScParticle(
      x: rng.nextDouble(),
...
class _ScShimmerPainter extends CustomPainter {
  _ScShimmerPainter({required this.progress});
  ...
  @override
  bool shouldRepaint(covariant _ScShimmerPainter oldDelegate) =>
      oldDelegate.progress != progress;
}

class _ScParticlePainter extends CustomPainter {
  _ScParticlePainter({required this.particles, required this.phase});

  final List<_ScParticle> particles;
  final double phase;
  ...
  @override
  bool shouldRepaint(covariant _ScParticlePainter oldDelegate) =>
      oldDelegate.phase != phase;
}
