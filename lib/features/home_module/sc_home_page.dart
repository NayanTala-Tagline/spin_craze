...
// Home native ad disabled — not needed on this screen.
// import '../onboarding_module/widgets/sc_ad_slot.dart';
...
///
/// The screen background and bottom navigation are owned by `ScBottomNavPage`,
...
class _ScHomePageState extends State<ScHomePage> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      ...
      builder: (context, _) {
        final provider = context.read<ScHomeProvider>();
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _ScStatTilesRow(level: level, xp: xp),
                SizedBox(height: AppSize.h20),
                // Home native ad disabled — not needed on this screen.
                // ScAdSlot(ad: _homeNativeAd),
                // SizedBox(height: AppSize.h16),
                const _ScHowItWorksBanner(),
                SizedBox(height: AppSize.h20),
                const _ScLeaderboard(),
                SizedBox(height: AppSize.h16),
              ],
            ),
          ),
...
class _ScHomeAppBar extends StatelessWidget implements PreferredSizeWidget {
  const _ScHomeAppBar({
    required this.coins,
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                _ScStreakChip(days: streak, onTap: onStreakTap),
              ],
            ),
          ),
          Container(
            ...
            child: CustomPaint(
              ...
              painter: ScPremiumDividerPainter(),
            ),
          ),
        ],
      ),
...
class ScPremiumDividerPainter extends CustomPainter {
  @override
...
class _ScStreakChip extends StatelessWidget {
  const _ScStreakChip({required this.days, this.onTap});
...
class _ScDailyRewardCard extends StatelessWidget {
  const _ScDailyRewardCard({
    required this.coins,
...
class _ScPaleCyanButton extends StatelessWidget {
  const _ScPaleCyanButton({required this.label, required this.onPressed});
...
class _ScTotalBalanceCard extends StatelessWidget {
  const _ScTotalBalanceCard({
...
class _ScOutlineWhiteButton extends StatelessWidget {
  const _ScOutlineWhiteButton({required this.label, required this.onPressed});
...
class _ScStatTilesRow extends StatelessWidget {
  const _ScStatTilesRow({required this.level, required this.xp});
...
class _ScStatTile extends StatelessWidget {
  const _ScStatTile({
    required this.label,
    required this.value,
    required this.icon,
...
class _ScEarnModulesCarousel extends StatefulWidget {
  const _ScEarnModulesCarousel();

  @override
  State<_ScEarnModulesCarousel> createState() => _ScEarnModulesCarouselState();
}

class _ScEarnModulesCarouselState extends State<_ScEarnModulesCarousel> {
      ...
      children: [
        ...
        SizedBox(
          ...
          child: PageView.builder(
            ...
            itemBuilder: (_, i) =>
                _ScEarnFeatureCard(data: items[i % items.length]),
          ),
        ),
        SizedBox(height: AppSize.h14),
        Align(
          alignment: Alignment.center,
          child: _ScPageIndicator(count: items.length, currentIndex: _cardIndex),
        ),
      ],
    );
  }
...
class _ScEarnItemData {
  const _ScEarnItemData(
    this.title,
    this.subtitle,
    this.image,
    this.icon, {
...
class _ScEarnIconAvatar extends StatelessWidget {
  const _ScEarnIconAvatar({required this.icon, required this.isActive});
...
class _ScEarnFeatureCard extends StatelessWidget {
  const _ScEarnFeatureCard({required this.data});

  final _ScEarnItemData data;
...
class _ScHeroCarousel extends StatefulWidget {
  const _ScHeroCarousel({
  ...
  @override
  State<_ScHeroCarousel> createState() => _ScHeroCarouselState();
}

class _ScHeroCarouselState extends State<_ScHeroCarousel> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: AppSize.h14),
        _ScPageIndicator(count: 2, currentIndex: _index),
      ],
    );
  }
}
...
class _ScPageIndicator extends StatelessWidget {
  const _ScPageIndicator({required this.count, required this.currentIndex});
...
class _ScHowItWorksBanner extends StatelessWidget {
  const _ScHowItWorksBanner();
...
class _ScPaleCyanLearnButton extends StatelessWidget {
  const _ScPaleCyanLearnButton({required this.onPressed});
...
class _ScLeaderboard extends StatelessWidget {
  const _ScLeaderboard();
