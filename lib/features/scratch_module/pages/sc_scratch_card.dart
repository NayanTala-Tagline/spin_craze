...
import 'package:spin_craze/features/scratch_module/provider/sc_scratch_card_provider.dart';
...
class ScScratchCardScreen extends StatefulWidget {
  const ScScratchCardScreen({super.key});

  @override
  State<ScScratchCardScreen> createState() => _ScScratchCardScreenState();
}

class _ScScratchCardScreenState extends State<ScScratchCardScreen>
  ...
  void initState() {
    ...
    AnalyticsManager.instance.logScreenView(
      ...
      screenClass: 'ScScratchCardScreen',
    );
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => ScScratchCardProvider(),
      child: PopScope(
        canPop: false,
        ...
        child: Scaffold(
          ...
          body: SafeArea(
            top: false,
            child: Consumer<ScScratchCardProvider>(
...
class _ScScratchArea extends StatelessWidget {
  const _ScScratchArea({
    required this.provider,
    required this.shakeController,
    required this.shakeAnimation,
  });

  final ScScratchCardProvider provider;
                  ...
                  child: Column(
                    ...
                    children: [
                      if (!provider.isGiftBoxOpened)
                        ...
                        _ScRevealedReward(reward: provider.reward ?? 0),
                    ],
                  ),
                ),
              ),
  ...
  void _showResultSheet(int reward) {
    ...
    showModalBottomSheet<void>(
      ...
      isDismissible: false,
      enableDrag: false,
      builder: (sheetCtx) => _ScCongratsSheet(
        coins: reward,
        isLoss: isLoss,
        onClaim: () async {
          sheetCtx.pop();
...
class _ScRevealedReward extends StatelessWidget {
  const _ScRevealedReward({required this.reward});
...
class _ScCongratsSheet extends StatelessWidget {
  const _ScCongratsSheet({
    required this.coins,
