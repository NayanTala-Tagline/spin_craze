...
const _badgeBg = Color(0xFFEAF1FF);

/// Onboarding step 2/3 — pick your currency (single-select, searchable).
class ScCurrencyPage extends StatefulWidget {
  const ScCurrencyPage({super.key, required this.onContinue, this.preloaded});

  /// Called on "Next"; receives the pre-loaded ads for the next screen.
  final void Function(ScSelectionAds? next) onContinue;

  /// Ads pre-loaded by the previous screen (country).
  final ScSelectionAds? preloaded;

  @override
  State<ScCurrencyPage> createState() => _ScCurrencyPageState();
}

class _ScCurrencyPageState extends State<ScCurrencyPage> {
  ...
  void initState() {
    ...
    AnalyticsManager.instance.logScreenView(
      screenName: 'onboarding_currency',
      screenClass: 'ScCurrencyPage',
    );
    ...
    _ads = ScSelectionAdProvider(
      ...
      nativeData: RemoteConfigService.instance.currencyNative,
      interData: RemoteConfigService.instance.currencyInter,
      // Pre-load the next screen (games) while this one is shown.
      nextNativeData: RemoteConfigService.instance.gameSelectNative,
      nextInterData: RemoteConfigService.instance.gameSelectInter,
  ...
  List<ScCurrencyOption> get _filtered {
  ...
  Future<void> _onNext() async {
    ...
    AnalyticsManager.instance.logEvent(
      name: 'onboarding_currency_selected',
      parameters: {'currency': _selected ?? 'unknown'},
    );
    await _ads.wait();
...
class _ScCurrencyTile extends StatelessWidget {
  const _ScCurrencyTile({
    required this.currency,
    required this.selected,
    required this.onTap,
  });

  final ScCurrencyOption currency;
  final bool selected;
            ...
            children: [
              ...
              if (selected)
                Icon(Icons.check_circle,
                    color: Colors.white, size: AppSize.sp22),
            ],
          ),
        ),
      ),
