...
class ScProfilePage extends StatefulWidget {
  const ScProfilePage({super.key});

  @override
  State<ScProfilePage> createState() => _ScProfilePageState();
}

class _ScProfilePageState extends State<ScProfilePage>
  ...
  Future<void> _handleSignOut() async {
    ...
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => _ScSignOutDialog(),
    );
    if (confirmed != true) return;
    if (!mounted) return;

    final auth = ScAuthProvider();
    await auth.signOut();
    if (mounted) {
  ...
  Future<void> _handleDeleteAccount() async {
    ...
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => _ScDeleteAccountDialog(),
    );
    if (confirmed != true) return;
    if (!mounted) return;
            ...
            child: Column(
              ...
              children: [
                ...
                _ScStaggered(
                  ...
                  child: _ScLevelLine(level: level, tier: tier),
                ),
                ...
                _ScStaggeredRow(
                  controller: _entrance,
                  index: 5,
                  child: _ScRateUsRow(onTap: _handleRateUs),
                ),
                _ScStaggeredRow(
                  controller: _entrance,
                  index: 6,
                  child: _ScDeleteAccountButton(onTap: _handleDeleteAccount),
                ),
                SizedBox(height: AppSize.h8),
                _ScStaggeredRow(
                  controller: _entrance,
                  index: 7,
                  child: _ScSignOutButton(onTap: _handleSignOut),
                ),
...
class _ScStaggeredRow extends StatelessWidget {
  const _ScStaggeredRow({
    required this.controller,
    required this.index,
    required this.child,
  });
...
class _ScProfileAvatar extends StatelessWidget {
  const _ScProfileAvatar({required this.initial, this.photoUrl});
    ...
    return Container(
      ...
      child: hasPhoto
          ? Image.network(
              ...
              errorBuilder: (_, _, _) => _ScAvatarInitial(initial: initial),
            )
          : _ScAvatarInitial(initial: initial),
    );
  }
}

class _ScAvatarInitial extends StatelessWidget {
  const _ScAvatarInitial({required this.initial});
...
class _ScLevelLine extends StatelessWidget {
  const _ScLevelLine({required this.level, required this.tier});
...
class _ScStatsRow extends StatelessWidget {
  const _ScStatsRow({this.coins = 0, this.xp = 0, this.streak = 0});
  ...
  @override
  Widget build(BuildContext context) {
    final stats = <_ScStatTileData>[
      _ScStatTileData(
        context.l10n.profileBalance,
        '\$${(coins / RemoteConfigService.instance.coinToDollarDivider).toStringAsFixed(3)}',
      ),
      _ScStatTileData(context.l10n.homeStatXp, '$xp'),
      _ScStatTileData(context.l10n.profileDays, '$streak'),
      _ScStatTileData(context.l10n.profileRefs, '0'),
    ];

    return Row(
      children: [
        for (var i = 0; i < stats.length; i++) ...[
          Expanded(child: _ScStatTile(data: stats[i], index: i)),
...
class _ScStatTileData {
  const _ScStatTileData(this.label, this.value);
  final String label;
  final String value;
}

class _ScStatTile extends StatelessWidget {
  const _ScStatTile({required this.data, required this.index});

  final _ScStatTileData data;
  final int index;
...
class _ScLevelProgressCard extends StatelessWidget {
  const _ScLevelProgressCard({required this.percent, required this.animation});
...
class _ScSettingNavRow extends StatelessWidget {
  const _ScSettingNavRow({
    required this.icon,
    required this.label,
    required this.onTap,
  });
...
class _ScSettingRowShell extends StatelessWidget {
  const _ScSettingRowShell({
    required this.icon,
    required this.label,
...
class _ScSignOutButton extends StatelessWidget {
  const _ScSignOutButton({required this.onTap});
...
class _ScRateUsRow extends StatelessWidget {
  const _ScRateUsRow({required this.onTap});
...
class _ScDeleteAccountButton extends StatelessWidget {
  const _ScDeleteAccountButton({required this.onTap});
...
class _ScDeleteAccountDialog extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final colors = context.themeColors;
    final textColors = context.themeTextColors;
...
class _ScSignOutDialog extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final textColors = context.themeTextColors;
