...
typedef _QuestionGenerator = _ScQuestion Function(Random rng);

/// Builds a question with shuffled options. Returns the [_ScQuestion] with
/// the correct answer placed at a random index.
_ScQuestion _buildQuestion(
  Random rng,
  String text,
...
_ScQuestion _genMultiply(Random rng) {
...
// 2. Solve for x: x + a = b
_ScQuestion _genSolveAdd(Random rng) {
...
// 3. Division (clean)
_ScQuestion _genDivide(Random rng) {
...
// 4. Percentage
_ScQuestion _genPercent(Random rng) {
...
_ScQuestion _genSqrt(Random rng) {
...
_ScQuestion _genOrderOps(Random rng) {
...
_ScQuestion _genFraction(Random rng) {
...
// 8. Solve for y: a * y = b
_ScQuestion _genSolveMul(Random rng) {
...
_ScQuestion _genSquareMinus(Random rng) {
...
_ScQuestion _genDecimal(Random rng) {
...
List<_ScQuestion> _generateQuestions() {
  final rng = Random();
...
class ScQuizScreen extends StatefulWidget {
  const ScQuizScreen({super.key});

  @override
  State<ScQuizScreen> createState() => _ScQuizScreenState();
}

class _ScQuizScreenState extends State<ScQuizScreen> {
  late final List<_ScQuestion> _questions = _generateQuestions();
  int _currentIndex = 0;
  int? _selectedOption;
  bool _answered = false;
  int _correctCount = 0;

  _ScQuestion get _current => _questions[_currentIndex];
  ...
  void _showResultSheet() {
    ...
    showModalBottomSheet<void>(
      ...
      isDismissible: false,
      enableDrag: false,
      builder: (sheetCtx) => _ScResultSheet(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _ScQuestionCard(question: _current.question),
                  SizedBox(height: AppSize.h32),
...
class _ScProgressFooter extends StatelessWidget {
  const _ScProgressFooter({required this.current, required this.total});
...
class _ScQuestionCard extends StatelessWidget {
  const _ScQuestionCard({required this.question});
...
class _ScOptionTile extends StatelessWidget {
  const _ScOptionTile({
    required this.text,
    required this.index,
...
class _ScResultSheet extends StatelessWidget {
  const _ScResultSheet({
